...
def create_heat_kernel(xx, yy, t):
    kernel = (1/(2*np.pi))*np.exp(-(np.sqrt(xx**2 + yy**2)**2)/(4*np.expand_dims(t, axis=(1, 2))))
    return kernel
...
def main():
    ...
    x, y = range(np.shape(img_pad)[0]), range(np.shape(img_pad)[1])
    ...
    f_gaussian = get_fourier_transform(gaussian)
    f_sol = f_img * f_gaussian
